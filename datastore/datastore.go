...
func (ds *datastore) List(key string, kvObject KVObject) ([]KVObject, error) {
	...
	err := ds.populateFromStore(key, kvObject, cb)
	if err != nil {
		return nil, err
	}
	return kvol, nil
...
func (ds *datastore) populateFromStore(key string, kvObject KVObject, callback func(string, KVObject)) error {
...
func (ds *datastore) Map(key string, kvObject KVObject) (map[string]KVObject, error) {
	...
	if ds.cache != nil {
		m, err := ds.cache.kmap(kvObject)
		return map[string]KVObject(m), err
	}
