...
func (n *network) addSvcRecords(eid, name string, epIP net.IP, epIPv6 net.IP, ipMapUpdate bool, method string) {
	...
	logrus.Warnf("%s (%s).addSvcRecords(%s, %s, %s, %t) %s", eid, n.ID()[0:7], name, epIP, epIPv6, ipMapUpdate, method)
...
func (n *network) deleteSvcRecords(eid, name string, epIP net.IP, epIPv6 net.IP, ipMapUpdate bool, method string) {
	...
	logrus.Warnf("%s (%s).deleteSvcRecords(%s, %s, %s, %t) %s", eid, n.ID()[0:7], name, epIP, epIPv6, ipMapUpdate, method)
...
func (n *network) ResolveIP(ip string) string {
	...
	if !ok {
		logrus.Warnf("Something is wrong with the ipInfo set for key %s set:%s", ip, sr.ipMap.String(ip))
		return ""
	}
